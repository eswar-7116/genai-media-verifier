...
export const PillBase: React.FC = () => {
    ...
    // Scroll detection - use GSAP ScrollTrigger callbacks for features section
    useEffect(() => {
        if (isTransitioning) return;

        // Check if features section is pinned by GSAP
        ...
        const observerCallback = (entries: IntersectionObserverEntry[]) => {
            // Check features section first
            checkFeaturesActive();
            
            const featuresSection = document.getElementById('features');
            const isFeaturesPinned = featuresSection?.getAttribute('data-features-active') === 'true';
            
            if (isFeaturesPinned) return; // Don't update if features is pinned
            ...
            entries.forEach(entry => {
                // Skip features section in normal intersection observer
                if (entry.target.id === 'features') return;
            ...
            // Update active section to the one with highest visibility
            if (maxEntry && maxRatio > 0.05) {
